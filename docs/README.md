...
SpiritsInObjects will accept images and video of any dimensions, but for the purposes of making sound-on-film video, 16:9 is the ideal aspect ratio.
...
Keep this in mind when adding images and unless you have a specific reason for doing otherwise, try to keep all of your images in the same resolution and aspect ratio for use with the Composer.
...
Audio files created in mono or single-channel [WAV](https://en.wikipedia.org/wiki/WAV)<sup>[3](#footnote3)</sup> format are supported by default with no need for additional transcoding or mixdown.

All other audio files will be interpreted as mono, single-channel files no matter what is provided to the software.
On opening an MP3 or multi-channel WAV file, the application will transcode and mix down your file to a temporary file stored by the app.
...
The tone generated by a single frame can be added in groups, sustaining the sound from a single image for multiple frames.
...
![SCREENSHOT OF FRAMES DRAGGED INTO THE TIMELINE]()
...
Videos exported from the Composer view come in a `.mov` [QuickTime](https://en.wikipedia.org/wiki/QuickTime_File_Format)<sup>[4](#footnote4)</sup> container and are encoded as [Apple ProRes 444](https://www.apple.com/final-cut-pro/docs/Apple_ProRes_White_Paper.pdf)<sup>[5](#footnote5)</sup>.
...
Videos can be sonified, previewed and exported by first dragging them from your operating system's file browser or desktop into the application.
...
Still images can be sonified using a similar process as with videos but with fewer features.
...
When using a video, scrubbing along the timeline will change the count in the input box

Scrubbing through a video can be done by clicking the timeline or dragging the cursor.
Frame by frame scrubbing can be accomplished by pressing the "Prev Frame" and "Next Frame" buttons on either side of the play button.
...
![SCREENSHOT OF THE VISUALIZE TOOL]()

The Visualize tool enables you to create visualizations of a MIDI or audio file that, when committed to 16mm film, will produce the same sound when played in a 16mm projector.
The application does this by generating images that represent notes in MIDI files or by interpreting audio sample data using the [SoundtrackOptical.js](https://github.com/sixteenmillimeter/SoundtrackOptical.js)<sup>[6](#footnote6)</sup> library.
...
Once loaded, a frames will be generated for the first track by default.
The preview will jump ahead in the video timeline to the first rendered note.
...
![SCREENSHOT OF VIDEO RESOLUTION DROPDOWN]()
...
![SCREENSHOT OF TRACK DROPDOWN]()

Different tracks can be selected in the dropdown that appears below the preview window.
Labelled "Track #, Instrument [#]" the different tracks in your MIDI file will all be presented as options with their instrument and instrument ID number.
Changing the selection in this dropdown will cause your preview video to render the newly-chosen track.

![SCREENSHOT OF WAVE TYPE DROPDOWN]()

The MIDI audio can also be represented with "Square Wave" (the default) bars representing the tone of a note or "Sine Wave" gradients.
Changing this value triggers a re-render of your preview.

![SCREENSHOT OF OFFSET DROPDOWN]()
...
This is because the optical soundtrack reader on a 16mm projector sits 26 frames after the projection gate, so the image is shown before it is read by the optical reader head.
...
![SCREENSHOT DEMONSTRATING MIDI WITH OFFSET]()

<a name="visualizing_audio"></a>

**ii. Visualizing audio**

Similar to MIDI files, standard audio can also be visualized to play back when committed to film and played in a 16mm projector.
...
![SCREENSHOT OF SOUNDTRACK TYPE DROPDOWN]()

When using the "Variable density (full screen)" soundtrack type, the sample data of your audio file will appear stretched completely across the 16:9 frame at different densities that, when played back, will produce a facsimile of the original audio.

![SCREENSHOT OF VARIABLE DENSITY OFFSET]() 
...
![SCREENSHOT OF DIFFERENT SOUNDTRACK TYPES]()

These different types are detailed more completely in the original Processing library that SoundtrackOptical.js was based on, [SoundtrackOptical](https://github.com/sixteenmillimeter/SoundtrackOptical)<sup>[7](#footnote7)</sup> and in [a guide created by Paul Ivester](http://www.paulivester.com/films/filmstock/guide.htm)<sup>[8](#footnote8)</sup>.
...
Because these images are generated on the fly based on your settings, anytime you change a setting such as the selected track, the wave style, the soundtrack type or the offset you must create a new preview.
...
**Note:** Opening or dropping an save file into the application while you have unsaved changes will overwrite them.
This may cause you to lose work if you have not already saved your work in a `.sio` file.
